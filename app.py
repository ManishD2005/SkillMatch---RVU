...
def dashboard():
    ...
    return render_template('dashboard.html', internships=internships, user_type=user_type)
...
def apply(internship_id):
    ...
    if request.method == 'POST':
        ...
        db = get_db()
        db.execute('INSERT INTO applications (student_id, internship_id, name, email, skills, motivation) VALUES (?, ?, ?, ?, ?, ?)', 
                   (student_id, internship_id, name, email, skills, motivation))
        db.commit()
